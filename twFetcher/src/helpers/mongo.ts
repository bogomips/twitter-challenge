...
mongo.getDocs = async (collection:string, searchObj,projectObj,SLS = {sort:{},limit:0,skip:0}) =>  {

	const {sort,limit,skip} = SLS;	
	return mongo.state.connRef.db(mongo.dbName).collection(collection).find(searchObj,projectObj).sort(sort).limit(limit).skip(skip);
}

mongo.insertMany = async (collection, docs, ordered) =>  mongo.state.connRef.db(mongo.dbName).collection(collection).insertMany(docs,{ordered:ordered});	
